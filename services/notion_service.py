...
class NotionService:
    ...
    async def create_run_log_entry(self, event: str, status: str, details: str, document_id: str | None = None) -> dict[str, Any]:
        ...
        if not self.settings.run_log_id:
        ...
        payload = {
            "parent": {"database_id": self.settings.run_log_id},
        ...
        try:
            ...
            # We don't want a run log failure to completely kill the workflow, so we just return empty
            return {}
